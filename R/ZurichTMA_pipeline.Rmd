...
#Extract relevant PhenoGraph columns from saved out PG run
```{r}
...
my.formula <- y ~ x
ggplot(plot_dat, aes_string(x="IHC", y="IMC" ,color = "Antigen"))+
  geom_point(aes(alpha=1,size=0.3))+
  geom_smooth(method = "lm")+
  stat_poly_eq(formula = my.formula, 
                aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
                parse = TRUE) +theme_bw()
...
#Summary across all correlations for Fig1
```{r}
#Get R squares separately
Rsq = data.frame()
for (i in unique(plot_dat$Antigen)){
  cur = plot_dat[Antigen == i,]
  res = lm(IMC ~ IHC, data=cur)
  Rsq = rbind(Rsq,cbind(i,summary(res)$r.squared))
}

Rsq$V2 = as.numeric(as.character(Rsq$V2))
Rsq$i = factor(Rsq$i, levels = c('ER','PR','HER2','Ecadherin','KI67'))


pdf('/home/jana/Desktop/R_dat/Rsq.pdf')
ggplot(Rsq,aes(y = V2,x = i,fill = i))+
  geom_bar(stat = "identity")+
  scale_fill_manual(values = mycols_basel_meta[c(23,24,20,22,16)])+
  ylim(0,1)+
  xlab('Marker')+
  ylab('R squared')
dev.off()
...
save = comb[,c('core','IMC_pos_count','IHC_count','marker')]
...
save = rbind(save,comb[,c('core','IMC_pos_count','IHC_count','marker')])
...
#Difference in pos counts
```{r}

#On top of each other
my.formula <- y ~ x

save$marker = factor(save$marker, levels = c('ERpos','PRpos','HER2','Ecadh','Ki67pos'))
pdf('/home/jana/Desktop/R_dat/counts.pdf')
ggplot(save, aes_string(x="IHC_count", y="IMC_pos_count" ,color = "marker"))+
  geom_point(alpha=0.3,size=2)+
  scale_color_manual(values = mycols_basel_meta[c(23,24,20,22,16)])+
  geom_smooth(method = "lm")+
  stat_poly_eq(formula = my.formula, 
                aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
                parse = TRUE) +theme_bw()
...
#Differences in pos cell counts
save[,diff:= abs(IMC_pos_count - IHC_count)]

pdf('/home/jana/Desktop/R_dat/diff_counts.pdf')
ggplot(save,aes(y = diff,x = marker))+
  geom_violin()
dev.off()
...
#B and A plots for positive cells only across cores of same patient
```{r}
...
datER = merge(datER,unique(Sample_metadata[,c('core','PID')]),by = 'core')
...
datER = unique(datER[,c('PID','core','mean_core','ncells','ncells_pos')])

#only use cores with at least 200 cells
datER = datER[ncells > 200,]


datER[,mean_patient := mean(mean_core), by = c('PID')]
