...
missing = patient_cluster_all$patientcode[!patient_cluster_all$patientcode %in% frag$patientcode]
frag = rbind(frag,data.frame(patientcode = missing,frag_score = rep(NA,length(missing)),MeanSize = rep(NA,length(missing)),MeanSize_log = rep(NA,length(missing)),core = rep(NA,length(missing))))
frag = frag[frag$patientcode %in% patient_cluster_all$patientcode,]
frag[,patient_frag := mean(frag_score),by = 'patientcode']
frag = unique(frag[,c('patientcode','patient_frag')])
...
nodules = fread('/home/jana/Desktop/R_dat/nodules_epi_basel.csv',header = T) #nodules_tumor.csv

#Threhsold 20
nodules[,size_comm := .N, by = c('Community','core')]
nodules = nodules[size_comm > 9,] # increase to >19 for sbp of mixed tumors
...
#Image mean
nodules <- unique(nodules[,c("core","Pheno","ncells","Community")]) # perc_cluster instead of ncells for mixed nodules part
nodules$Pheno = factor(nodules$Pheno, levels = sort(unique(nodules$Pheno)))

#Make missing cells types 0
nodules_wide = dcast.data.table(nodules,formula = 'Community + core ~ Pheno',value.var = 'ncells',fill = 0) # perc_cluster instead of ncells for mixed nodules part
...
#Set not present images NA
missing = patient_cluster_all$patientcode[!patient_cluster_all$patientcode %in% comm_kldiv$patientcode]
comm_kldiv = rbind(comm_kldiv,data.frame(patientcode = missing,kldiv = rep(NA,length(missing)),core = rep(NA,length(missing))))
comm_kldiv = comm_kldiv[comm_kldiv$patientcode %in% patient_cluster_all$patientcode,]
comm_kldiv[,patient_kl := mean(kldiv),by = 'patientcode']
comm_kldiv = unique(comm_kldiv[,c('patientcode','patient_kl')])

#Order according to Basel
comm_kldiv =  comm_kldiv[order(match(comm_kldiv$patientcode,patient_cluster_all$patientcode))]


threshold = quantile(comm_kldiv$patient_kl, probs = 0.95,na.rm = T)
m = mean(comm_kldiv$patient_kl)
std1 = std(comm_kldiv$patient_kl)

#Binary
comm_kldiv$bin[comm_kldiv$patient_kl < threshold] = 0
comm_kldiv$bin[comm_kldiv$patient_kl >= threshold] = 1
...
g = ggplot(comm_kldiv, aes(x = patient_kl))+
  geom_density()+
...
#fwrite(nodules_wide[,c('Community','cluster')],'/home/jana/Desktop/R_dat/PG_basel_epi_final.csv',col.names = T)

cl_dat = fread('/home/jana/Desktop/R_dat/PG_basel_epi_final.csv',header = T) #PG_comm_epik80_inclSize.csv
...
#fwrite(tsne_comm$Y,'/home/jana/Desktop/R_dat/tsne_basel_epi_final.csv',col.names = F)

tsne_comm = fread('/home/jana/Desktop/R_dat/tsne_basel_epi_final.csv') #tsne_comm_epik80_inclSize.csv
...
p = dat_tsne%>%
  ...
  labs(colour="Patients")+
  scale_color_manual(values = mycols_basel_meta[c(21,15,26,20,19,17,20,14,18,25,27,24,24,22,19,22,23,20,25,16,25,22,23)][as.numeric(hr$labels[hr$order])])+
  scale_alpha_manual(values=c(1,1,1,1,1,1,0.5,1,1,1,1,1,0.5,1,0.5,0.3,0.5,0.3,0.3,1,0.5,0.5,1)[as.numeric(hr$labels[hr$order])])+
  ggtitle('Phenograph')+
  guides(color=guide_legend(override.aes=list(size=5)))+
  theme(strip.background = element_blank(),
        panel.background=element_rect(fill='white', colour = 'black'),
...
pdf('/home/jana/Desktop/R_dat/tsne_epithelial_communities_colorMajority.pdf',width = 10,height = 10)
p
dev.off()
...
pdf("/home/jana/Desktop/R_dat/tsne_celltype.pdf",width = 30,height = 10)
p
dev.off()
...
meta_patient_clustering = rbind(meta_patient_clustering,data.frame(patientcode = c('UB_SX_000429','UB_SX_000153','UB_SX_000443'), patient_pheno = c(18,18,18)))
...
d = merge(d, meta_patient_clustering,by = 'patientcode')
...
pdf('/home/jana/Desktop/R_dat/levels_ordered_sbp_and_epiHeatmap.pdf',width = 10, height = 20)
h
dev.off()
...
surv = merge(unique(Sample_metadata[,c('patientcode','OSmonth','Patientstatus')]),surv,by = 'patientcode')

surv = surv[!is.na(surv$OSmonth),]
colnames(surv)[-c(1:3)] = paste0('cluster_',colnames(surv)[-c(1:3)])

#Change Unit so the values aren't all super small and to push non-zeros above 1
...
#Assumptions
dfbeta = residuals(res.cox2, type='dfbeta')
qplot(dfbeta) + theme_bw()
which.influence(res.cox2, cutoff=.2)
outlier = which(dfbeta > 0.05)
surv[outlier]

prop_hazards = cox.zph(res.cox2)
ggcoxdiagnostics(res.cox2, type = , linear.predictions = TRUE)
ggcoxdiagnostics(res.cox2, type = "deviance",
                 linear.predictions = FALSE, ggtheme = theme_bw())
ggcoxfunctional(res.cox2)

library(mgcv)
fit <- gam(Surv(surv$OSmonth, surv$censoring) ~ s(cluster_1, bs = 'cr', k = 10) + s(cluster_2, bs= 'cr', k = 10) +
           s(cluster_3, bs = 'cr', k = 10), family = cox.ph(), data = surv)
