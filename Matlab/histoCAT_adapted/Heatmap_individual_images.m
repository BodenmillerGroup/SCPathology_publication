function [Matrix_Delta,Matrix_low,Unique_all,Unique_low_all,Matrix_high,Unique_high_all,pheno_name,Unique_all_string]...
...
% Generate neighborhood matrix, accumulate across each image
...
%Get the patientcode according to the image index
meta = readtable('/home/jana/Desktop/bb_volume_jana/Data/2018/Clinical_paper/SpilloverCorrected/R_dat/20180129_renamed_Metadata.csv');
meta = meta(~contains(table2array(meta(:,1)),'NA_NA'),:);
%meta = meta(contains(table2array(meta(:,'core')),'ZTMA'),:);
...
Delta = clustergram(Matrix_Delta_cut_noNaN,'RowLabels',gates(selectedall_gates',1)...
    ...
    num2str(pixelexpansion),'_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information,'_p-value',num2str(pVal_sig)]);


writeout = array2table(Matrix_Delta_cut_noNaN,'VariableNames',strcat('Variable',Unique_all_string_names_cut),'RowNames',gates(selectedall_gates',1));
writetable(writeout,'ZuriRevisionsClustergram.csv','WriteRowNames',true);

% %Make background actually white
...
saveas(plot_delta,fullfile(custom_gatesfolder,['Clustergram_all_Pixel',num2str(pixelexpansion),...
    '_Perm_',num2str(permutations),'_',pheno_name,'_',Extra_information,'_p-value',num2str(pVal_sig),'.fig']));
...
higher_special = clustergram(Matrix_cut_special_noNaN,'RowLabels',gates(selectedall_gates,1),'ColumnLabels',Special_for_columnlabel_cut,...
    'Cluster','all','linkage','ward','Colormap',redbluecmap); 

addTitle(higher_special,['Delta_Special_Pixel',num2str(pixelexpansion),'_Perm_',permutations,'_',pheno_name,'_',Special_clusters_name,'_',Extra_information,'_p-value',num2str(pVal_sig)]);

plot_delta_special =plot(higher_special);

% Save special output clustergram
saveas(plot_delta_special,fullfile(custom_gatesfolder,['Clustergram_special_Pixel',num2str(pixelexpansion),'_Perm_',num2str(permutations),'_',pheno_name,'_SpecialCluster',Special_clusters_name,'_',Extra_information,'_p-value',num2str(pVal_sig),'.fig']));
