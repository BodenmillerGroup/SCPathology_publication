...
% PLOT_MASK_CALLBACK: slightly adapted from original histoCAT version, can
% just be plugged in to replace original function. Adapted to allow for
% plotting of multiple masks.
...
if handles.mask_onoff.Value == 1
    ...
    if isempty(maskoutline) ~= 1
        ...
        %Catch exceptions and generate mask outlines for current image
            ...
            %cell is marked with the corresponding cell number)
            try
                lblImg_filled = Mask_all(idxfound_name).Image{1};
            catch
                lblImg_filled = Mask_all(1,idxfound_name).Image;
            end
