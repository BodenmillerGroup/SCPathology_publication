...
% SHOW_SELECTED_AREA_ONTIFF: Slightly adapted to our case with our
% customized cell type colors and to not display legend on top of image.
...
cluster_names = gates(SGsof_imageids_open,1);
split_name = cellfun(@(x) strsplit(x,'_'),cluster_names,'UniformOutput',false);
split_name = cellfun(@(x) strcat(x{2}), split_name,'UniformOutput',false);
cluster_num = str2double(split_name);
...
for ik = 1:length(imageids)
    ...
    if isempty(Fcs_Interest_all{sample_orderIDX(ik),1}) ~= 1
        ...
        try
            lblImg = Mask_all(1,sample_orderIDX(ik)).Image{1};
        catch
        ...
        for sesn = SGsof_imageids_open
            ...
            %Import colors previously exported from R pipeline to have
            %consistent color code
            basel_meta_cols = readtable('/home/jana/Desktop/R_dat/RGB_basel_meta.csv');
            basel_meta_cols = table2array(basel_meta_cols);
            basel_meta_cols = [basel_meta_cols;[0,0,0]];

            [ rgbimg_rest ] = make_rgb( tempImg,basel_meta_cols,cluster_num(count));
        ...
        %Store count numbers
        %countnum = [countnum cluster_num(count-1)];
        ...
        if isempty(commoncells) ~= 1
            ...
            [ rgbimg ] = make_rgb( tempImg,basel_meta_cols,cluster_num(count));
            ...
            %Remove legends because when batch saving images we didn't want
            %the legend to hide parts of the image.
...
%             set(L,{'color'},mat2cell(basel_meta_cols(countnum,:),ones(1,numel(countnum)),3))
...
%             set(L,{'color'},mat2cell(basel_meta_cols(cluster_num(countnum),:),ones(1,numel(countnum)),3))
