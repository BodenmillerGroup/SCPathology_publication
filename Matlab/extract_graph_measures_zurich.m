...
%Retrieve session Data from histoCAT
gates = retr('gates');
...
for image_num = 1:353
    ...
    idx_tumor_pheno = ismember(Phenograph_Vector_orig, [4,6,8:15,17,19:20,22:27,29:38,40]);
...
try
    ...
    niter = 20;
    if ispc == 1	
        [c,Q,labels,communities] = LouvainfromBin_Windows(fullfile(custom_gates,'S.bin'),niter,'Yes');	
    else	
...
writetable(mean_size,'fragmentation_zurich.csv');
%Write out community data for analysis in R
writetable(label_out,'nodules_stroma_zurich.csv');
...
for image_num = 1:353
    ...
    %Get single-cell data of current image
    rows = sessionData(gates{image_num,2},2);
    selectedall_gates = image_num;
    
    %Get neighbor info of single-cells
    pixelexpansion = 4;
    expansion_name = ['neighbour_',num2str(pixelexpansion),'_CellId'];   
    neigb_index = cellfun(@(x) find(~cellfun('isempty',regexp(x,expansion_name))),...
    gates(selectedall_gates,3),'UniformOutput',false);  
